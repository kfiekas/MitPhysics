// $Id: PUReweigthing.cc,v 1.7 2011/05/13 08:27:16 ceballos Exp $


#include "MitPhysics/Utils/interface/PUReweigthing.h"
#include <TFile.h>

ClassImp(mithep::PUReweigthing)

using namespace mithep;

double PUReweigthing::reweightOOT(int inTimePUMultiplicity, int outOfTimePUMultiplicity, const char* fileName, const char*histoName) {

  TFile *referenceFile = TFile::Open(fileName);
  if (!referenceFile) {
    std::cout<<"Error: PU reference file does not exist, reweigthing will not be applied"<<std::endl;
    return 1.0;
  }

  TH1F * referenceHisto = (TH1F*) referenceFile->Get(histoName);
  if (!referenceHisto) {
    std::cout<<"Error: PU reference histo does not exist in the file, reweigthing will not be applied"<<std::endl;
    return 1.0;
  }
