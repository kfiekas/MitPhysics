// $Id: PUReweighting.cc,v 1.1 2011/05/26 12:00:56 mzanetti Exp $


#include "MitPhysics/Utils/interface/PUReweighting.h"
#include <TFile.h>

ClassImp(mithep::PUReweighting)

using namespace mithep;


PUReweighting::PUReweighting(const char* fileName, const char*histoName) {

  TFile *referenceFile = TFile::Open(fileName);
  if (!referenceFile) {
    std::cout<<"Error: PU reference file does not exist, reweighting will not be applied"<<std::endl;
  }
  else {
    referenceHisto = (TH1F*) referenceFile->Get(histoName);
  }

}

double PUReweighting::reweightOOT(int inTimePUMultiplicity, int outOfTimePUMultiplicity) {

    if (!referenceHisto) {
      std::cout<<"Error: PU reference histo does not exist in the file, reweighting will not be applied"<<std::endl;
      return 1;
    }

  // residual correction to match lumi spectrum
  static double Correct_Weights2011[25] = { 
    5.30031,
    2.07903,
    1.40729,
    1.27687,
  ...
  for(int iint = 0; iint<25; ++iint){
    ...
    for(int ibin = 0; ibin<25; ++ibin){
      WeightOOTPU[iint][ibin] = *(WeightPtr+ibin);
    }
  }


  double inTimeWeight = referenceHisto->GetBinContent( inTimePUMultiplicity );

  return inTimeWeight * WeightOOTPU[inTimePUMultiplicity-1][outOfTimePUMultiplicity-1] * Correct_Weights2011[inTimePUMultiplicity-1];
