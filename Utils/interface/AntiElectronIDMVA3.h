...
//
// Authors: I.Naranjo
// Ported to BAMBU by Matthew Chan (11/30/12)
//--------------------------------------------------------------------------------------------------

#ifndef MITPHYSICS_UTILS_AntiElectronIDMVA3_H
#define MITPHYSICS_UTILS_AntiElectronIDMVA3_H
...
namespace mithep
{
  ...
  public:
    ...
    double MVAValue(const PFTau& thePFTau, 
		    const Electron& theGsfEle);
    // CV: this function can be called for category 1 only !!
    double MVAValue(const PFTau& thePFTau);

    bool Pass(double mva, int category, Level level=Loose);
    bool Pass(const PFTau *thePFTauRef, const ElectronCol *electrons, double &mva, int &category, Level level=Loose);
    bool Pass(const PFTau *thePFTauRef, const ElectronCol *electrons, Level level=Loose);
